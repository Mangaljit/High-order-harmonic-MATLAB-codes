...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
folder=fullfile('E:\PhD data\Experimetal Data\Beamtime 10 Hz July2017\tin7_18jul2017');
cd(folder);
ref_img=fitsread('tinspectrum1_0001.fts'); 
imshow(ref_img, [110 250]); %[0 500] is just for the contrast management
...
for j=1:length(angle_hwp);
      h1(j,:)=sum((img_plot_series_1st(j,xval(1):xval(2)))-min(img_plot_series_1st(j,xval(1):xval(2))));
end
% for j=1:length(angle_hwp);
...
sum((img_plot_series_1st(j,xval(1):xval(2)))-min(img_plot_series_1st(j,xval(1):xval(2))))

plot(angle_hwp,abs(h1./max(h1)),'o-','Linewidth',3);
% ylim([0 1.1])
%set(gca, 'YScale', 'log')
