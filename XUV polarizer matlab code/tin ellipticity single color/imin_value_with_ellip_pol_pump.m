...
%keep imshow(ref_img, [100 170]) same everywhere
% two spectrums, one with elliptic s-pol and other with elliptic p-pol is uploaded.
...
samples=2;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Spectrum for elliptic s-polarization
...
%Spectrum for elliptic p-polarization
folder=fullfile('C:\Users\Mangaljit Singh\Desktop\indium5_2aug2017');
cd(folder);
...
imin=h1p./h1s %this aray gives the value of Rp for individual harmonic




% rs=1; %intensity with s-polarized linear pump
% rp=0.3; %intensity with p-polarized linear pump
% imax=1; %intensity with s-polarized elliptic pump
% imin=array_Rp; %intensity with p-polarized elliptic pump
% ellipticities=sqrt((rs.*imin-rp.*imax)./(rs.*imax-rp.*imin))



rs=1; %intensity with s-polarized linear pump
rp=0.5; %intensity with p-polarized linear pump and comes from separate code
imax=1; %intensity with s-polarized elliptic pump
ellipticities=sqrt((rs.*imin-rp.*imax)./(rs.*imax-rp.*imin))
